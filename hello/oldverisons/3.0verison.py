...
class RewardSystem:
    ...
    def evaluate_response(self, confidence):
        if confidence > self.positive_threshold:
            self.reward_score += 1  # Positive reward
            return 1
        elif confidence < self.negative_threshold:
...
#deletes start and end tokens new mothod delte if not working
    ...
    def generate_seq2seqresponse(self, input_sentence, num_candidates=5, temperature=0.9):
...
if os.path.exists("/Users/blakeweiss/Desktop/hello/conversations.txt") and os.path.exists("/Users/blakeweiss/Desktop/hello/answers.txt"):
...
#extract the conversations and save to files
    with open(conversations_file, 'r', encoding='iso-8859-1') as f, \
         open('/Users/blakeweiss/Desktop/hello/conversations.txt', 'w', encoding='utf-8') as conv_file, \
         open('/Users/blakeweiss/Desktop/hello/answers.txt', 'w', encoding='utf-8') as ans_file:
        for line in f:
            parts = line.split(' +++$+++ ')
...
with open('/Users/blakeweiss/Desktop/hello/conversations.txt', 'r', encoding='utf-8') as conv_file:
    input_texts = conv_file.readlines()
    

with open('/Users/blakeweiss/Desktop/hello/answers.txt', 'r', encoding='utf-8') as ans_file:
...
reward_system = RewardSystem(positive_threshold=0.3, negative_threshold=0.2)
...
(text for text in all_texts), target_vocab_size=2**13)
    ...
    while True:
        ...
        #many issues
        elif user_input.lower() == 'reward up':
            howmuchupinput = input('By how much would you like the reward to increase by: ')
            howmuchup = int(howmuchupinput)
            reward_system.reward_score += howmuchup
            #confidenceincrease = input('How much would you like to increase confidence by: ')
            #confidenceincreaseint = int(confidenceincrease)
            #confidence += confidenceincreaseint
            print("Total reward:", reward_system.get_total_reward())
            
        elif user_input.lower() == 'reward down':
            howmuchdowninput = input('By how much would you like the reward to decrease by: ')
            howmuchdown = int(howmuchdowninput) 
            reward_system.reward_score -= howmuchdown
            #confidencedecrease = input('How much would you like to decrease confidence by: ')
            #confidencedecreaseint = int(confidencedecrease)
            #confidence -= confidencedecreaseint
            print("Total reward:", reward_system.get_total_reward())
            ...
            reward = reward_system.evaluate_response(confidence)
